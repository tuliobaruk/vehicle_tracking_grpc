...
const TRACKING_SERVICE_HOST =
  process.env.TRACKING_SERVICE_HOST;
const TRACKING_SERVICE_PORT = process.env.TRACKING_SERVICE_PORT;

// Construir as URLs completas para os serviços
...
const etaClient = new etaProto.eta.ETAService(
  ETA_SERVICE_URL,
...
const LOCATIONS: Record<string, Location> = {
  recife: { lat: -8.0476, lon: -34.877, name: "Centro do Recife" },
  boaviagem: { lat: -8.1137, lon: -34.8851, name: "Boa Viagem" },
  olinda: { lat: -8.0089, lon: -34.8553, name: "Olinda" },
  jaboatao: { lat: -8.1137, lon: -35.022, name: "Jaboatão Centro" },
...
function formatETA(eta: any): void {
  ...
  console.log(
    `│ 📍 Posição atual: ${eta.currentLat?.toFixed(
      6
    )}, ${eta.currentLon?.toFixed(6)}`
  );
  ...
  console.log(
    `│ 🕐 Chegada: ${new Date(eta.arrivalTime).toLocaleString("pt-BR")}`
  );
...
function showMainMenu(): void {
  ...
  console.log(
    "🚗 Veículos conectados:",
    connectedVehicles.length > 0 ? connectedVehicles.join(", ") : "Nenhum"
  );
  console.log("");
  console.log("╔═══════════════════════════════════════════════════════════╗");
...
function showLocations(): void {
  ...
  Object.entries(LOCATIONS).forEach(([code, loc]) => {
    ...
    console.log(
      `│ ${code.padEnd(14)} │ ${loc.name.padEnd(23)} │ ${coords.padEnd(19)} │`
    );
  });
...
function calculateSingleETA(vehicleId: string): void {
  ...
  etaClient.CalculateETA(request, (error: any, response: any) => {
    ...
    formatETA(response);
    console.log(
      `\n🕐 Calculado em: ${new Date(response.calculatedAt).toLocaleString(
        "pt-BR"
      )}`
    );
  });
...
function calculateAllETAs(): void {
  ...
  etaClient.GetMultipleETAs(request, (error: any, response: any) => {
    ...
    console.log(
      `\n🏆 RANKING DE CHEGADA (${etas.length} veículo${
        etas.length > 1 ? "s" : ""
      }):`
    );
...
function compareVehicles(): void {
  ...
  etaClient.GetMultipleETAs(request, (error: any, response: any) => {
    ...
    console.log(
      "│ Veículo    │ Dist(km)│ Vel(km/h)│ ETA(min)│ Chegada             │"
    );
    console.log(
      "├────────────┼─────────┼──────────┼─────────┼─────────────────────┤"
    );
    ...
    etas.forEach((eta: any) => {
      ...
      console.log(
        `│ ${vehicle} │ ${distance} │ ${speed} │ ${time} │ ${arrival.padEnd(
          19
        )} │`
      );
    });
    ...
    const sortedBySpeed = [...etas].sort(
      (a, b) => b.currentSpeed - a.currentSpeed
    );
    const sortedByDistance = [...etas].sort(
      (a, b) => a.distanceKm - b.distanceKm
    );

    console.log("\n🔍 ANÁLISES:");
    console.log(
      `🚀 Mais rápido: ${sortedBySpeed[0].vehicleId} (${sortedBySpeed[0].currentSpeed} km/h)`
    );
    console.log(
      `📍 Mais próximo: ${sortedByDistance[0].vehicleId} (${sortedByDistance[0].distanceKm} km)`
    );
    console.log(
      `⏱️  Chegará primeiro: ${etas[0].vehicleId} (${etas[0].estimatedMinutes} min)`
    );
  });
}
    ...
    case "eta":
      if (args.length === 0) {
        if (connectedVehicles.length === 0) {
          console.log(
            '❌ Nenhum veículo conectado. Use "list" para verificar.'
          );
        } else {
